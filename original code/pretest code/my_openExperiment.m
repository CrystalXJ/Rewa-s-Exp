function screenInfo = my_openExperiment(monWidth, viewDist, curScreen)
%
% Modified from openExperiment (under OSXDots/).  
%
...
rseed = sum(100*clock);
rand('state',rseed);
% Each time openExperiment is called, it resets the state used to generate
% random number using rand according to the current time.
% This can avoid generating the same sequence of random numbers.
...
% [screenInfo.curWindow, screenInfo.screenRect] = Screen('OpenWindow',curScreen,screenInfo.bckgnd,[0 0 2*640 1.5*480],32, 2);
...
screenInfo.frameDur = 1000/screenInfo.monRefresh;   %frame duration in msec
